...
.section .muted { color: var(--ink3) !important; font-size: 0.9rem; margin-top: 0.35rem; }
.section p, .section li, .section td, .section th { color: var(--ink2); }
.section h2, .section h3 { color: var(--ink); }
...
def generate_jmeter_ab_comparison_html(analysis: Dict[str, Any]) -> str:
    ...
    page_header = render_report_header(
        ...
        (
            f"Load testing comparison &amp; executive analysis · {current_date} · "
            f"<strong>{_esc(name_a)}</strong> (baseline) vs <strong>{_esc(name_b)}</strong> (candidate) · "
            f"Generated {now_utc}"
        ),
        icon_class="ti ti-arrows-left-right",
        icon_tone="pu",
        actions_html=pdf_button,
    )
    ...
    footer_html = f"""
        <footer class="foot report-foot report-footer-meta">
            <strong>Generated:</strong> {_esc(rf.get("generated_date", current_date))}
            <div class="fdv"></div>
            <strong>Generated by:</strong> {_esc(rf.get("generated_by", "Raghvendra Kumar"))}
            <div class="fdv"></div>
            <strong>Classification:</strong> {_esc(rf.get("classification", "Internal — Performance testing"))}
        </footer>
    """
...
<body>
{render_report_shell_open(AB_COMPARISON_REPORT_NAV)}
{page_header}
{render_report_body_open()}
        <div class="traffic-legend colour-legend no-print">
        ...
        <div id="report-executive" class="report-anchor executive-summary exec-traffic-{_esc(exec_signal)}">
        ...
        <div id="report-release" class="report-anchor section report-readable">
            <h2>Business impact analysis &amp; release decision</h2>
            {business_html}
        </div>

        <div id="report-distribution" class="report-anchor section report-readable">
            <h2>Statistical distribution analysis</h2>
            <div style="padding:16px">{dist_html}</div>
        </div>

        <div id="report-grades" class="report-anchor section report-readable">
            <h2>Grade comparison</h2>
            <p class="muted">Overall and by dimension (baseline → candidate). Higher scores are better.</p>
            {grades_html}
        </div>

        <div id="report-metrics" class="report-anchor section report-readable">
        ...
        <div id="report-charts" class="report-anchor section report-readable">
        ...
        <div id="report-conclusion" class="report-anchor section section-conclusion-traffic-{_esc(exec_signal)} report-readable">
            <h2>Conclusion</h2>
