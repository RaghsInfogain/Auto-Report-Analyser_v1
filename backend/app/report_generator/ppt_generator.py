...
class PPTReportGenerator:
    ...
    def generate_jmeter_ppt_report(metrics: Dict[str, Any]) -> bytes:
        ...
        PPTReportGenerator.add_section_slide(prs, "📊 Executive Summary", exec_items)
        ...
        title_frame.text = "🎯 Performance Scorecard & Grading"
        ...
        title_frame = title_box.text_frame
        title_frame.text = "📖 Category Summary"
        title_para = title_frame.paragraphs[0]
        ...
        PPTReportGenerator.add_section_slide(prs, "📏 Grading Methodology", method_items)
        ...
        if critical_issues:
            ...
            PPTReportGenerator.add_section_slide(prs, "🔴 Critical Issues", issue_items)
        ...
        if improvement_roadmap:
            ...
            PPTReportGenerator.add_section_slide(prs, "🎯 Path to A+ Grade", improve_items)
        ...
        PPTReportGenerator.add_section_slide(prs, "🚀 Recommended Action Plan", action_items)
        ...
        title_box = slide.shapes.add_textbox(Inches(0.5), Inches(0.3), Inches(9), Inches(0.5))
        title_frame = title_box.text_frame
        title_frame.text = "🎯 Success Metrics & Targets"
        title_para = title_frame.paragraphs[0]
        title_para.font.size = Pt(28)
        title_para.font.bold = True
        title_para.font.color.rgb = RGBColor(37, 99, 235)
        ...
        if recommendations:
            ...
            PPTReportGenerator.add_section_slide(prs, "💡 Recommendations", rec_items)
        ...
        PPTReportGenerator.add_section_slide(prs, "📞 Next Steps & Contacts", next_items)
    ...
    def generate_web_vitals_ppt_report(metrics: Dict[str, Any], filename: str = "web_vitals_report") -> bytes:
        ...
        # Title Slide
        PPTReportGenerator.add_title_slide(prs, "⚡ Web Vitals Performance Report", f"Core Web Vitals Analysis\n{current_date}\nSamples: {total_samples:,}")
        ...
        summary_items = [
            ...
            {"text": f"LCP (Largest Contentful Paint): {lcp_mean:.0f}ms {'✅ Good' if lcp_mean <= 2500 else '⚠️ Needs Work' if lcp_mean <= 4000 else '❌ Poor'}", "level": 1},
            {"text": f"FID (First Input Delay): {fid_mean:.0f}ms {'✅ Good' if fid_mean <= 100 else '⚠️ Needs Work' if fid_mean <= 300 else '❌ Poor'}", "level": 1},
            {"text": f"CLS (Cumulative Layout Shift): {cls_mean:.3f} {'✅ Good' if cls_mean <= 0.1 else '⚠️ Needs Work' if cls_mean <= 0.25 else '❌ Poor'}", "level": 1},
            {"text": f"FCP (First Contentful Paint): {fcp_mean:.0f}ms {'✅ Good' if fcp_mean <= 1800 else '⚠️ Needs Work' if fcp_mean <= 3000 else '❌ Poor'}", "level": 1},
            {"text": f"TTFB (Time to First Byte): {ttfb_mean:.0f}ms {'✅ Good' if ttfb_mean <= 800 else '⚠️ Needs Work' if ttfb_mean <= 1800 else '❌ Poor'}", "level": 1},
            {"text": "", "level": 0},
        ...
        PPTReportGenerator.add_section_slide(prs, "📊 Web Vitals Summary", summary_items)
    ...
    def generate_ui_performance_ppt_report(metrics: Dict[str, Any], filename: str = "ui_performance_report") -> bytes:
        ...
        # Title Slide
        PPTReportGenerator.add_title_slide(prs, "🎯 UI Performance Report", f"Page Load Timing Analysis\n{current_date}\nSamples: {total_samples:,}")
        ...
        PPTReportGenerator.add_section_slide(prs, "📊 Performance Summary", summary_items)
        ...
        PPTReportGenerator.add_section_slide(prs, "📈 95th Percentile Analysis", p95_items)
